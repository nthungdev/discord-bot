...
/**
 * Resolves the GenerativeModel instance configured with tools if available.
 */
function resolveVertexModel(
  vertexAI: VertexAI,
  ...
  const systemInstruction =
    config.systemInstruction + (config.membersInstruction || "");
...
/**
 * Builds the array of message parts from prompt text and file attachments.
 */
async function buildVertexMessageParts(
  prompt: AiPrompt,
): Promise<(InlineDataPart | TextPart)[]> {
  const { text, files = [] } = prompt;
...
/**
 * Executes a single function call in Vertex AI provider.
 */
async function executeVertexSingleToolCall(
...
/**
 * Runs the tool invocation loop for Vertex AI.
 */
async function runVertexToolCallingLoop(
...
export class VertexGenAi implements GenAi {
  ...
  async generate(prompt: AiPrompt): Promise<AiPromptResponse> {
    ...
    const parts = await buildVertexMessageParts(prompt);

    try {
      const initialResult = await chat.sendMessage(parts);
      const finalResult = await runVertexToolCallingLoop(
        chat,
        prompt,
        initialResult,
      );
      const content = extractVertexResponseContent(finalResult);
