...
- **Manual Configuration Management**: System configurations (e.g. `guildEmojis`, `guildMembers`, `bots` policies, AI model selection, safety parameters) must be manually edited directly in the Firebase Remote Config console or modified in `config.json`. There is no schema validation, rollback history, or visual editing interface.
