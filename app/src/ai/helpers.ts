...
export const getContext = () => {
  // TODO: experiment to see whethere the bot still pick up the first bestie on the list to mention or not
  const shuffledBesties = besties.sort(() => Math.random() - 0.5);
  const bestiesString = shuffledBesties.map(({ name, gender, username }) => `${name} (gender: ${gender}, username: @${username})`).join(', ');
