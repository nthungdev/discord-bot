...
export async function countCheckInsInChannel(channelId: string, start: Date, end: Date) {
  ...
  while (!done) {
    await channel.messages.fetch({ limit: 100, before: lastMessage?.id }).then(messages => {
      for (const [_, message] of messages) {
        ...
        else if (message.type === MessageType.ChatInputCommand &&
          message.interaction?.commandName !== 'checkin'
        ) {
          continue
        }
        else if (!isWithinPeriod(start, end, message.createdAt)) {
          console.log(`not within the period ${start.toLocaleString()} - ${end.toLocaleString()}`)
          done = true
          break
        }
  ...
  messageBuffer.forEach(message => {
    ...
    const user = message.type === MessageType.ChatInputCommand
      ? message.interaction!.user
      : message.author
    const streakStatus = checkStreak(message.createdAt, tracker[user.id]?.lastCheckIn)
    const currentStreak = streakStatus === 'increment'
      ? (tracker[user.id]?.currentStreak || 0) + 1
      : streakStatus === 'reset'
        ? 1
        : (tracker[user.id]?.currentStreak || 0) // same date
    const longestStreak = Math.max(currentStreak, tracker[user.id]?.longestStreak || 0)
...
export function formatCheckInLeaderboard(startDate: Date, endDate: Date, leaderboard: [string, CheckInTrackerData][]) {
  ...
  const longestStreakLeaderboard = leaderboard
    .toSorted((a, b) => b[1].longestStreak - a[1].longestStreak)
  ...
  const report = config.checkInReportTemplate
    ...
    .replace('{longestStreakCount}', longestStreakLeaderboard[0][1].longestStreak.toString())
